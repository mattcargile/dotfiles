...
1. rip out setting of `$env:Path` in `$PROFILE` and set it in a run after on change script. consider template or `.chezmoidata`. might be able to manually derive the pathing separators because the paths are delinated on platform
