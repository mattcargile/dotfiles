...
Set-PSReadLineKeyHandler -Chord 'Alt+%' -BriefDescription 'ExpandAliases' -Description "Replace all aliases with the full command (and expand parameters where unambiguous)" -ScriptBlock {
    param($key, $arg)
...
Set-PSReadLineKeyHandler -Chord 'Alt+a' -Function SelectCommandArgument -ViMode Command
