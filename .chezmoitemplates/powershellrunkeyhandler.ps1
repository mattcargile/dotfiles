Set-PSReadLineKeyHandler -BriefDescription 'PowerShellRunTabCompletion' -Description 'Tab completes anything' -Chord 'Ctrl+8,Tab' -ScriptBlock {
...
Set-PSReadLineKeyHandler -BriefDescription 'PowerShellRunSearchPSReadLineHistory' -Description 'Searchs PSConsoleReadLine History Items' -Chord 'Ctrl+r' -ScriptBlock {
    param([ConsoleKeyInfo]$key, [object]$arg)
    # Force implicit module load at beginning
    $option = Get-PSRunDefaultSelectorOption
    $option.Theme.PreviewTextWrapMode = 'Character'
...
Set-PSReadLineKeyHandler -BriefDescription 'PowerShellRunSelectProviderItems' -Description 'Select files names from current directory as a parameter value' -Chord 'Ctrl+t' -ScriptBlock {
    param([ConsoleKeyInfo]$key, [object]$arg)
